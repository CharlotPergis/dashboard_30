...
def update_data():
    ...
    try:
        ...
        # RPi sends ONLY temperature and current (NO probabilities)
    ...
    # ============================================
    # FLASK CALCULATES EVERYTHING USING ML MODELS
    # ============================================
    ...
    # Current adjustment for low current
    if current < 16:
        ovl_prob *= 0.5
    ...
    if state in ["Warning", "Critical"]:
        ...
        if should_send_alert(alert_key):
            ...
            reading = SimpleNamespace(temperature_c=temp, current_a=current)
            send_breaker_alert(reading, {"hotspot_prob": hot_prob, "overload_prob": ovl_prob}, state, action)
        else:
...
def test_supabase():
    ...
    try:
        ...
        return jsonify({"success": True, "record_count": len(response.data) if hasattr(response, 'data') else 0})
    except Exception as e:
